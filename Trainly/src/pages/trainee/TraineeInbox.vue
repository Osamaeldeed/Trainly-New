<template>
  <div class="page-root flex items-start justify-center min-h-screen  rounded-2xl dark:bg-[#1c1c1c]  bg-blue-100 py-6">
    <!-- البوكس الكبير -->
    <div
    ...
    >
      ...
      >
        <div class="p-6 border-b border-gray-200">
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white dark:text-white mb-4">Chats</h1>
          <div class="relative">
            <span class="absolute left-3 top-1/2 transform dark:text-white -translate-y-1/2 text-gray-400">🔍</span>
            <input
              type="text"
              v-model="searchQuery"
            ...
            >
              ...
              <div class="flex-1 min-w-0 dark:text-white text-left">
                <div class="flex items-center justify-between mb-1">
                  <h3 class="font-semibold dark:text-white text-gray-900 truncate">{{ trainer.name }}</h3>
                  <span v-if="trainer.lastMessageTime" class="text-xs dark:text-white text-gray-500 flex-shrink-0 ml-2">
                    {{ formatTime(trainer.lastMessageTime) }}
                  </span>
                </div>
                <div class="flex items-center justify-between">
                  <p class="text-sm dark:text-white text-gray-600 truncate">{{ trainer.lastMessage || 'Start a conversation' }}</p>
                  <span
      ...
      >
        ...
        <!-- top bar for mobile -->
        <div v-if="isStacked" class="mobile-top bg-white dark:text-white dark:bg-[#3B3B3B] border-b border-gray-200 p-3 flex items-center gap-3">
          <button v-if="selectedTrainer" @click="mobilePane = 'trainers'" class="p-2 rounded-md dark:text-white hover:bg-gray-100">←</button>
          <div v-if="selectedTrainer" class="flex items-center gap-3">
            ...
            />
            <div>
              <div class="font-semibold dark:text-white text-gray-900 text-sm">{{ selectedTrainer.name }}</div>
              <div class="text-xs dark:text-white text-gray-500">{{ selectedTrainer.email || 'Trainer' }}</div>
            </div>
          </div>
        </div>

        <!-- desktop header -->
        <div v-else class="bg-white dark:bg-[#3B3B3B] dark:text-white border-b border-gray-200 p-4 flex items-center gap-3">
          <img
          ...
          />
          <div>
            <h2 class="font-semibold dark:text-white text-gray-900">{{ selectedTrainer ? selectedTrainer.name : 'Select a trainer' }}</h2>
            <p class="text-sm dark:text-white text-gray-500">{{ selectedTrainer ? (selectedTrainer.email || 'Trainer') : '' }}</p>
          </div>
        </div>

        <!-- messages area -->
        <div class="flex-1 flex flex-col">
          <div ref="messagesContainer" class="messages-container flex-1 overflow-y-auto p-6 space-y-4 dark:bg-[#3B3B3B]  bg-gray-50">
            <div v-if="!selectedTrainer" class="flex-1 flex items-center justify-center">
              <div class="text-center dark:text-white text-gray-400">
                <div class="w-24 h-24 bg-gray-200 dark:bg-[#3B3B3B]  rounded-full mx-auto mb-4 flex items-center justify-center">
                    ...
                    <div :class="['flex flex-col', msg.senderId === this.traineeId ? 'items-end ml-auto' : 'items-start mr-auto']" style="max-width:70%;">
                      <div
                        :class="[
                          ...
                          msg.senderId === this.traineeId ? 'bg-blue-600 text-black dark:text-white rounded-br-sm' : 'bg-white dark:bg-[#3B3B3B]  text-gray-900 rounded-bl-sm'
                        ]"
                      >
                        <p class="text-sm dark:text-white whitespace-pre-wrap">{{ msg.text }}</p>
                      </div>
                      <p class="text-xs dark:text-white text-gray-500 mt-2">{{ formatTime(msg.timestamp) }}</p>
                    </div>
                    <img
                      v-if="msg.senderId === this.traineeId"
