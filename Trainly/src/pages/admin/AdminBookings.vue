<template>
  <div>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
      <div>
        <h1 class="text-2xl dark:text-white font-semibold mb-1">Sessions & Bookings</h1>
        <p class="dark:text-gray-300 text-gray-500">Manage all training sessions and bookings</p>
      </div>
      ...
      <div class="relative w-full md:w-1/3">
        ...
        <input
          ...
          class="w-full pl-10 pr-3 dark:text-white py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
        />
      </div>
    </div>
    ...
    <!-- Table for Desktop -->
    <div
      class="hidden md:block bg-white dark:bg-[#292929] shadow-lg rounded-xl overflow-hidden border border-gray-200"
    >
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-blue-50 dark:bg-[#292929]">
          <tr>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Trainer</th>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Trainee</th>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Sport</th>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Plan</th>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Date</th>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Amount</th>
            <th class="px-6 py-3 dark:text-white text-gray-600 font-medium text-left">Status</th>
          </tr>
        </thead>
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          >
            ...
            <td class="px-6 py-4 dark:text-gray-300 text-gray-700">
              {{ booking.trainee?.firstName }} {{ booking.trainee?.lastName }}
            </td>
            <td class="px-6 py-4 dark:text-gray-300 text-gray-700">
              {{ booking.trainer?.sport || "N/A" }}
            </td>
            <td class="px-6 py-4 dark:text-gray-300 text-gray-700">
              {{ booking.plan?.title || "N/A" }}
            </td>
            <td class="px-6 py-4 dark:text-gray-300 text-gray-700">
              {{ formatDate(booking.createdAt?.seconds) }}
            </td>
            <td class="px-6 py-4 dark:text-gray-300 text-gray-700">
    ...
    <div class="md:hidden flex flex-col gap-4">
      ...
      >
        ...
        <!-- Booking Details -->
        <p>
          <span class="font-semibold m-2">Trainer:</span> {{ booking.trainer?.firstName }}
          {{ booking.trainer?.lastName }}
        </p>
        <p>
          <span class="font-semibold m-2">Trainee:</span> {{ booking.trainee?.firstName }}
          {{ booking.trainee?.lastName }}
        </p>
        ...
        <p>
          <span class="font-semibold m-2">Date:</span> {{ formatDate(booking.createdAt?.seconds) }}
        </p>
        <p>
          <span class="font-semibold m-2">Amount:</span> {{ booking.amount }} {{ booking.currency }}
        </p>
      </div>
    </div>
